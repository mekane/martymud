...
void do_mstat( CHAR_DATA *ch, char *argument )
{
    ...
    /* MEK added for religions */
    if (!IS_NPC(victim))
    {
      sprintf(buf,"Religion: %s\n\r", god_table[victim->religion].name);
      send_to_char(buf,ch);
    }
    ...
    /*MEK TEST - for morphing */
    if (!IS_NPC(victim))
    {
      sprintf(buf,"Morph Short Desc: %s\n\r", victim->short_desc_morph);
      send_to_char(buf,ch);
      sprintf(buf,"Morph Long Desc: %s\n\r", victim->long_desc_morph);
      send_to_char(buf,ch);
     sprintf(buf,"Morph Description: %s\n\r",victim->description_morph);
      send_to_char(buf,ch);
    }
    ...
    /* MEK TEST - for talkative mobs */

    if ( IS_NPC(victim) && victim->pIndexData->extra_descr != NULL)
    {
      EXTRA_DESCR_DATA *ed;
      ...
      /*
	for ( ed = victim->pIndexData->extra_descr; ed != NULL; ed = ed->next )
	{
	send_to_char( ed->keyword, ch );
	if ( ed->next != NULL )
	send_to_char( " ", ch );
	...
	sprintf( buf,
	    ...
	    paf->modifier,
	    paf->duration,
	    /* MEK 10-16-2004  account for affect2 slots */
...
bool obj_check (CHAR_DATA *ch, OBJ_DATA *obj)
{
	...
	|| (IS_TRUSTED(ch,SUPREME) && obj->level <= 20 && obj->cost <= 1000)
	|| (IS_TRUSTED(ch,DEITY)   && obj->level <= 10 && obj->cost <= 500)
	|| (IS_TRUSTED(ch,DEMI)    && obj->level <=  5 && obj->cost <= 250)
...
void do_clone(CHAR_DATA *ch, char *argument )
{
	...
	||  (mob->level >  5 && !IS_TRUSTED(ch,DEITY))
	||  (mob->level >  0 && !IS_TRUSTED(ch,DEMI))
...
void do_mset( CHAR_DATA *ch, char *argument )
{
    ...
    if (!str_prefix(arg2,"religion"))
    {
      ...
      if( god == 0 )
      {
	send_to_char("That is not a valid religion for the char.\n\r",ch);
	return;
      }
