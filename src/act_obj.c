...
void get_obj( CHAR_DATA *ch, OBJ_DATA *obj, OBJ_DATA *container )
{
    ...
    /* MEK 10-16-2004  For Morph - No EQ/INV */
    if ( IS_AFFECTED2(ch, AFF_MORPHED)  )
    {
      send_to_char( "You can't carry items in your current form.\n\r", ch );
      return;
    } 
...
void do_put( CHAR_DATA *ch, char *argument )
{
    ...
    if ( IS_AFFECTED2(ch, AFF_MORPHED)  )
    {
      send_to_char("You can't manipulate items in your current form.\n\r",ch);
      return;
    } 
...
void do_eat( CHAR_DATA *ch, char *argument )
{
    ...
    /**
     * Except that animals should be able to eat stuff!!!
     * Look on ground too?
     */
...
void wear_obj( CHAR_DATA *ch, OBJ_DATA *obj, bool fReplace )
{
    ...
    /* MEK 3-16-2004
     * Change to let players use EQ 10 levels or less above their level
     */
	...
	if (obj->item_type != ITEM_WEAPON )
	{
	  send_to_char("You can only wield weapons.\n\r",ch);
	  return;
	}
...
void do_second( CHAR_DATA *ch, char *argument )
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *weapon; //check for other items in the way
  ...
  //replace old dual wield weapon
  if ( !remove_obj( ch, WEAR_DUAL, TRUE ) )
    return;

  //They must have a first weapon
  ...
  /* MEK 3-20-2004
   *
   * no shields, held items, or dual wielding when already wielding a 
   * two-handed weapon
   */
  ...
  /*
   * Second weapons can't be as heavy as normal weapons
   */
  if (!IS_NPC(ch) && 
      get_obj_weight(obj)>(str_app[get_curr_stat(ch,STAT_STR)].wield*7) )
  {
    send_to_char( "It is too heavy for you to wield.\n\r", ch );
    return;
  }

  //check for dual wield weapon being two-handed
  ...
  //okay! go ahead and dual wield it now
...
void do_sacrifice( CHAR_DATA *ch, char *argument )
{
    ...
    if ( !ch->religion && !IS_IMMORTAL(ch) )
    {
      send_to_char( "You do not worship a god! To whom would you sacrifice?\n\r", ch );
      return;
    }
	...
	send_to_char(
	    "%g appreciates your offer and may accept it later.\n\r", ch );
	return;
    }
    ...
    if (silver == 1)
      act( "$g gives you one silver coin for your sacrifice.", 
	   ch, NULL, NULL, TO_CHAR );
    else
    {
      sprintf(buf,"%s gives you %d silver coins for your sacrifice.\n\r",
	      god_table[ch->religion].name, silver);
      send_to_char(buf,ch);
    }
...
/* MEK 3-20-2004
 * 
 * Added to make lore skill actually DO something!
 */
void do_lore( CHAR_DATA *ch, char *argument )
{
  ...
  if ( IS_IMMORTAL(ch) )
  {
    send_to_char("Why use lore? - try something cooler!\n\r", ch );
    return;
  }
  */
  ...
  if ( (!IS_NPC(ch) &&
	ch->level < skill_table[gsn_lore].skill_level[ch->class]) ||
       (get_skill(ch,gsn_lore)) < 1 )
  {
    send_to_char("You don't know any legends or lore.\n\r", ch );
    return;
  }
  ...
  if ( arg[0] == '\0' )
  {
    send_to_char( "Recall lore about what?\n\r", ch );
    return;
  }
  ...
  //start with base chance = skill * 3/4
  chance = 3 * get_skill(ch,gsn_lore) / 4;
  
  //bonuses for old age
  int years = get_age(ch) - 17;
  int ageBonus = 0;
  ageBonus += (years >= 1);
  ageBonus += (years >= 2);
  ageBonus += (years >= 5);
  ...
  int rag = class_lookup("ranger");  
  int dru = class_lookup("druid");
  int ttn = class_lookup("titan");
  int classBonus = 0;
  ...
  chance = UMIN( 100, chance + classBonus );
  
  /* Testing */
  if ( IS_IMMORTAL(ch) )
  {
    sprintf(buf,"Test Lore: %d = %d(base) + %d(age) + %d(wis) + %d(class)\n\r",
	    chance,
	    3 * get_skill(ch,gsn_lore) / 4,
	    ageBonus,
  ...
  {
    if ( chance > 85 )
      sprintf( buf,"The legends surrounding %s all say that is",
	       obj->name);
    else if ( chance > 75 )
      sprintf( buf,"Some of the legends surrounding %s speak of it being",
	       obj->name);
    else if ( chance > 55 )
      sprintf( buf,"The legends of %s are hazy, but mention that it is",
	       obj->name);

    send_to_char( buf, ch );
    /* from identify - modified for use here */

    const char *type  = "";  //store the type string to display
    const char *power = ""; //store the description of the item's power
    const char *worth = ""; //store the description of the item's worth
    const char *extra = ""; //store any description stuff
    ...
    switch ( obj->item_type )
    {
      ...
      case ITEM_STAFF: 
	type  = "a magical item";
	power = "of great power"; //based on level / obj values
	worth = "of great worth"; //based on worth / extras
	
	//make another check
	if ( chance > 85 )
	  extra = "has spells"; //format this nice - and make informative
	break;
      case ITEM_DRINK_CON:
    ...
    case ITEM_WEAPON:
      switch (obj->value[0])
      {
	case(WEAPON_EXOTIC) : type = "exotic weapon";	break;
	case(WEAPON_SWORD)  : type = "sword";	break;	
	case(WEAPON_DAGGER) : type = "dagger";	break;
	case(WEAPON_SPEAR)  : type = "spear";	break;
	case(WEAPON_STAFF)  : type = "staff";	break;
	case(WEAPON_MACE)   : type = "mace";	break;
	case(WEAPON_AXE)    : type = "axe";	break;
	case(WEAPON_FLAIL)  : type = "flail";	break;
	case(WEAPON_WHIP)   : type = "whip";	break;
	case(WEAPON_POLEARM): type = "polearm";	break;
	default		    : type = "mysterious weapon";	break;
      }
    ...
    sprintf( buf," %s of %s, and %s worth.", type, power, worth );
    send_to_char( buf, ch );
  ...
  {
    sprintf( buf, "You failed to recall any legends surrounding %s.\n\r",
	     obj->name );
    send_to_char( buf, ch );
