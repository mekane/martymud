...
/*
 * MEK 3-11-2004
 * updated to reflect new races, classes, and groups for new classes
 */
#define MAX_SOCIALS		  256
#define MAX_SKILL		  160
#define MAX_GROUP		   50
 ...
 * MEK 3-10-2004
 * MaxLevel to 80 
 * Chage Hero level and Immortal Levels
 */
#define MAX_LEVEL		 80
...
#define	CREATOR			(MAX_LEVEL - 1) //79
#define SUPREME			(MAX_LEVEL - 2) //78
#define DEITY			(MAX_LEVEL - 3) //77
#define DEMI			(MAX_LEVEL - 4) //76
#define AVATAR			(MAX_LEVEL - 5) //75
...
struct	class_type
{
   ...
   * Added remort-check
   */
    bool        remort_class;           /* is a remort-only class       */
};
...
struct race_type
{
   ...
   * Added remort-check
   */
    bool        remort_race;            /* is this race only for remorts?*/
};
...
struct god_type
{
  ...
  sh_int      temple;                   /* Vnum of temple       */
  sh_int      symbol;                   /* Vnum of holy symbol item */
  sh_int      relic;                    /* Vnum of religion's relic-item */

  sh_int      max_align;                /* The maximum alignment to follow */
  sh_int      min_align;                /* The minimum alignment to follow */
};
 ...
 * Defined in #MOBILES.
 */
#define MOB_VNUM_ZOMBIE_LESSER	   2	   
#define MOB_VNUM_ZOMBIE_NORMAL	   3   
#define MOB_VNUM_ZOMBIE_GREATER	   4   
...
#define ITEM_DUAL		(O)
#define ITEM_HOLD		(P)
#define ITEM_NO_SAC		(Q)
#define ITEM_WEAR_FLOAT		(R)
...
struct	mob_index_data
{
    ...
    sh_int		size;
    char *		material;
    EXTRA_DESCR_DATA *	extra_descr; /* MEK - for talkative mobs */
};
...
struct	char_data
{
    ...
    char *		long_descr;
    char *		description;
    /* MEK 10-16-2003  Descriptions shown when char is morphed */
    char *		short_desc_morph;
    char *		long_desc_morph;
    char *		description_morph;

    char *		prompt;
    char *		prefix;
    sh_int		group;
    sh_int		clan;
  /* MEK 3-14-2004  For religions */
    sh_int              religion; // see religion.c
    sh_int		sex;
    sh_int		class;
    sh_int		race;
    sh_int		level;
...
struct	pc_data
{
     ...
     * Added for remort
     */
    bool                confirm_remort; //like confirm_delete, but for ascent
    sh_int              incarnations; //track how many ascentions they've made
};
...
extern sh_int  gsn_body_control;
...
/* MEK 10-16-2004  Added checks for when a player is morphed */
#define PERS(ch, looker) ( can_see( looker, (ch) ) ?		 \
			   ( IS_NPC(ch) ?                        \
                             ch->short_descr :                   \
				( IS_AFFECTED2(ch,AFF_MORPHED) ? \
                                  (ch)->short_desc_morph : (ch)->name ) ) \
				 : "someone" )
...
extern	char *	const			title_table	[MAX_CLASS]
							...
							[2];
/* MEK 3-14-2004
 * For religions - see religion.c for definitions
 */
...
int	get_weapon_sn_dual args( ( CHAR_DATA *ch ) );
