...
void multi_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt )
{
    ...
    //Everyone gets at least one try at hitting
    ...
    //backstab check - no extra attacks
    ...
    //chance for second attack with off hand weapon
    if (get_eq_char (ch, WEAR_DUAL))
    {
      chance = get_skill(ch,gsn_second_attack)/2;
	...
	chance /= 2;
      ...
      //check second attack chance
      if ( number_percent() < chance )
      {
	//check dual wield skill
...
void one_hit( CHAR_DATA *ch, CHAR_DATA *victim, int dt, bool dual )
{
    ...
    //We want to give assasins a bonus on backstab
    int asa = class_lookup("assasin");

    if (dt == gsn_backstab)
    {
      if ( ch->class == asa ) //assasins get slightly better to-hit with bs
      {
	thac0 -= 12 * (100 - get_skill(ch,gsn_backstab));
      }
      else
      {
    ...
    if ( dt == gsn_backstab && wield != NULL) 
    {
      /* 
       * MEK 10-10-2004
       * Give bigger bonus for sharp weapons / dagger
       */
      
      //divide level by this amount for bonus: smaller = better
      int div = 12;

      switch(dam_type)
      {
	//piercing is best, slashing is next, bash/other are worst
	case(DAM_PIERCE) : div -= 6; break;
	case(DAM_SLASH)  : div -= 4; break;
	case(DAM_BASH)   : div -= 2; break;	
	default          : break;
      }; 
      
      //assasins get more bonus damage on backstab
      if ( ch->class == asa )
	div -= 2;
      
      dam *= 2 + (ch->level / div); 
      
      //small bonus for dagger
	    ...
	    bool show) 
{
    ...
    */
    ...
    /* MEK 3-20-2004
     * Not sure why damage is scaled down, but I'll try some other numbers
     */
    if ( dam > 99)
    {
      int olddam = dam;
      dam = (dam - 45)/2 + 45;
      /*test*/printf("  %d > 99  dam = %d\n\r",olddam,dam);  
    }
	...
	/*
	 * MEK 10-17-2004
	 * Here is the position update in fight.c
	 * Could add morph check here
	 */
...
bool is_safe(CHAR_DATA *ch, CHAR_DATA *victim)
{
	...
	{
	    send_to_char("I don't think %g would approve.\n\r",ch);
	    return TRUE;
	}
...
void make_corpse( CHAR_DATA *ch )
{
    ...
    if ( IS_NPC(ch) )
    {
      /**
       * MEK 10-9-2004
       * Check for undead - don't make new corpse for animated corpses!
       */
      if (IS_SET(ch->form,FORM_UNDEAD))
      {
	act( "With the negative energies that animated it gone, $n crumbles into dust.", ch, NULL, NULL, TO_ROOM );
	return;
      }
...
int xp_compute( CHAR_DATA *gch, CHAR_DATA *victim, int total_levels )
{
     ...
     *
     * MEK 3-25-2004
     * Upped most of these values a little
     *
     */
    switch (level_range)
    {
      ...
      case -4 :	base_exp =  36;		break;
      case -3 :	base_exp =  50;		break;
      case -2 :	base_exp =  67;		break;
      case -1 :	base_exp =  83;		break;
      case  0 :	base_exp = 100;		break;
      case  1 :	base_exp = 121;		break;
      case  2 :	base_exp = 143;		break;
      case  3 :	base_exp = 165;		break;
      case  4 :	base_exp = 182;		break;
    } 
...
void dam_message( CHAR_DATA *ch, CHAR_DATA *victim,int dam,int dt,bool immune )
{
    ...
    else if ( dam <=   7 ) { vs = "graze";	vp = "grazes";		}
    else if ( dam <=  12 ) { vs = "hit";	vp = "hits";		}
    else if ( dam <=  17 ) { vs = "injure";	vp = "injures";		}
    else if ( dam <=  22 ) { vs = "wound";	vp = "wounds";		}
    else if ( dam <=  28 ) { vs = "maul";       vp = "mauls";		}
    else if ( dam <=  34 ) { vs = "decimate";	vp = "decimates";	}
    else if ( dam <=  40 ) { vs = "devastate";	vp = "devastates";	}
    else if ( dam <=  47 ) { vs = "maim";	vp = "maims";		}
    else if ( dam <=  57 ) { vs = "MUTILATE";	vp = "MUTILATES";	}
    else if ( dam <=  68 ) { vs = "DISEMBOWEL";	vp = "DISEMBOWELS";	}
    else if ( dam <=  80 ) { vs = "DISMEMBER";	vp = "DISMEMBERS";	}
    else if ( dam <=  93 ) { vs = "MASSACRE";	vp = "MASSACRES";	}
    else if ( dam <= 107 ) { vs = "MANGLE";	vp = "MANGLES";		}
    else if ( dam <= 125 ) { vs = "*** DEMOLISH ***";
			     vp = "*** DEMOLISHES ***";			}
    else if ( dam <= 150 ) { vs = "*** DEVASTATE ***";
			     vp = "*** DEVASTATES ***";			}
    else if ( dam <= 180 ) { vs = "=== OBLITERATE ===";
			     vp = "=== OBLITERATES ===";		}
    else if ( dam <= 220 ) { vs = ">>> ANNIHILATE <<<";
			     vp = ">>> ANNIHILATES <<<";		}
    else if ( dam <= 270 ) { vs = "<<< ERADICATE >>>";
			     vp = "<<< ERADICATES >>>";			}
    ...
    /* MEK 3-21-2004
     * IMMORTALS get to see their damage
     */
