...
/*
 * Appears to be for showing long descriptions of characters in rooms
 *
 */
void show_char_to_char_0( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    /* MEK 10-16-2004 - For morphing: don't show titles */
    if ( !IS_AFFECTED2(victim, AFF_MORPHED)  )
    {
...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
    ...
    {
      /* MEK 10-16-2004  For morphing: show new desc*/
      if ( IS_AFFECTED2(victim, AFF_MORPHED)  )
	send_to_char( victim->description_morph, ch );
      else
    ...
    /* MEK 10-16-2004  For morphing: don't show eq/inv */
    if ( IS_AFFECTED2(victim, AFF_MORPHED)  )
      return;
...
void do_score( CHAR_DATA *ch, char *argument )
{
    ...
    /* MEK 1/2/2005 - remorts can see all the info */
    int remort = ( ch->pcdata->incarnations > 1 );

    /* print AC values */
    if (ch->level >= 25 || remort) 
    {	
    ...
    if ( !IS_IMMORTAL(ch) )
    {
      if (ch->religion == 0)
      {
	send_to_char( "You are an atheist.\n\r", ch);
      }
      else
      {
	/*DEBUG*/printf("Score: god=%s.\n\r",(god_table[ch->religion].name));
	sprintf( buf, "You worship %s.\n\r",(god_table[ch->religion].name));
	send_to_char( buf, ch);
      }
    }       

    if ( ch->level >= 10 || remort )
    {
...
void do_whois (CHAR_DATA *ch, char *argument)
{
	...
	if (!str_prefix(arg,wch->name))
	{
	    ...
	    /*
	     * MEK 3-11-2004
	     * Modified this to use defines from merc.h
	     */
