...
void fwrite_char( CHAR_DATA *ch, FILE *fp )
{
     ...
     * Added for morph
     */
    fprintf( fp, "Morph-SD  %s~\n", ch->short_desc_morph	 );
    fprintf( fp, "Morph-LD  %s~\n", ch->long_desc_morph	 );
    fprintf( fp, "Morph-DD  %s~\n", ch->description_morph );
...
bool load_char_obj( DESCRIPTOR_DATA *d, char *name )
{
	...
	switch (ch->level)
	{
	    case(40) : ch->level = 80;	break;  /* imp -> imp */
	    case(39) : ch->level = 78; 	break;	/* god -> supreme */
	    case(38) : ch->level = 77;  break;	/* deity -> god */
	    case(37) : ch->level = 76;  break;	/* angel -> demigod */
	}

        switch (ch->trust)
        {
            case(40) : ch->trust = 80;  break;	/* imp -> imp */
            case(39) : ch->trust = 78;  break;	/* god -> supreme */
            case(38) : ch->trust = 77;  break;	/* deity -> god */
            case(37) : ch->trust = 76;  break;	/* angel -> demigod */
            case(36) : ch->trust = 75;  break;	/* hero -> hero */
        }
    }
...
void fread_char( CHAR_DATA *ch, FILE *fp )
{
	...
	case 'A':
	    ...
	    KEY( "AffectedByy2",ch->affected2_by,	fread_flag( fp ) );
	...
	case 'M':
	    KEYS( "Morph-SD", ch->short_desc_morph,  fread_string( fp ) );
	    KEYS( "Morph-LD", ch->long_desc_morph,   fread_string( fp ) );
	    KEYS( "Morph-DD", ch->description_morph, fread_string( fp ) );
	...
	case 'R':
	    ...
	    KEY( "Religion",    ch->religion,
		 god_lookup(fread_string( fp )) );
