...
/* MEK 3-20-2004 
 * Get the sn of a second weapon
 *
 * for returning weapon information 
 */
int get_weapon_sn_dual(CHAR_DATA *ch)
{
    OBJ_DATA *wield;
    int sn;
...
void char_from_room( CHAR_DATA *ch )
{
    ...
    //MEK 1/2/2005 Check empty stuff!
...
CHAR_DATA *get_char_room( CHAR_DATA *ch, char *argument )
{
	...
	continue;
      ...
      /* MEK 10-16-2004  Check for morphed character names too */
      if ( IS_AFFECTED2(rch, AFF_MORPHED) )
      {
	if (!is_name( arg, rch->short_desc_morph))
	  continue;
      }
      else
      {
...
char *affect2_bit_name( int vector )
{
    ...
    buf[0] = '\0';
    if ( vector & AFF_MORPHED       ) strcat( buf, " morphed"         );
